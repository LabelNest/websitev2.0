...
export async function POST(req: NextRequest) {
  ...
  const row = await sql`
    INSERT INTO website_fellows (name,role,cohort,department,linkedin_url,image_url,sort_order,is_active)
    VALUES (${m.name},${m.role||'Nestling Fellow'},${m.cohort||''},${m.department||null},${m.linkedin_url||null},${m.image_url||null},${m.sort_order||99},${m.is_active!==false})
    RETURNING id`
